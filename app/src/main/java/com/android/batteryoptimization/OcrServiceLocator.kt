...
/**
 * OCR 服务寻址助手：通过 DRouter 按接口类获取 [IOcrService] 实现。
 *
 * 解耦关键：:app 完全不 import :ocr_module 的任何类，只依赖 :ocr_api 中的接口。
 * 当 :ocr_module 未集成（gradle.properties 中 useOcr=false）时，DRouter 找不到对应 Service，
 * 返回 null —— 主 module 据此优雅降级（OCR 能力关闭），但其余功能照常运行。
 */
object OcrServiceLocator {
    fun get(): IOcrService? = try {
        DRouter.build(IOcrService::class.java).getService()
    } catch (e: Throwable) {
        null
    }
}
